# to sample VHH structures (overrides dataset filters):
# inference.samples.use_vhh_dataset=True (requires inpainting)

python cogeneration/scripts/predict.py \
 shared.seed=2468 \
 inference.task=inpainting \
 shared.id=heteromer_interface_stoch02 \
 dataset.inpainting.strategy=binding_interface_single_chain \
 inference.samples.length_step=16 \
 ...
 dataset.filter.num_unique_seqs=[2] \
